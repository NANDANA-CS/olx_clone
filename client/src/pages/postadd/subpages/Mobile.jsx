...
//       const response = await axios.post("http://localhost:3000/api/mobile", formDataToSend, {
//         headers: {
...
//                 disabled={loading}
//                 className={`w-full min-h-[40px] xs:min-h-[44px] py-1.5 xs:py-2 sm:py-2.5 md:py-3 bg-blue-600 text-white text-xs xs:text-sm sm:text-base font-medium rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
...
const Mobile = () => {
  ...
  const handlePostAd = async () => {
    ...
    try {
      ...
      formDataToSend.append('category', 'mobile');
      ...
      const errorMessage = error.response?.status === 404
        ? 'Server endpoint not found. Please check if the backend is running on http://localhost:3000/api/mobile.'
        ...
        <div className="w-full max-w-screen-lg min-w-[280px]">
          ...
          <div className="bg-white rounded-lg shadow-md p-2 xs:p-4 sm:p-6">
            ...
            <div className="mb-3 xs:mb-4 sm:mb-5">
              ...
              <div className="p-1 xs:p-2">
                <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-4 md:grid-cols-4 gap-1 xs issuing:gap-2 sm:gap-3">
                  {images.map((image, index) => (
                    <div key={index} className="relative">
                      {image ? (
                        <div>
